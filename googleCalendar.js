...
export async function addEventToCalendar(accessToken, schedule, workplace) {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `캘린더 이벤트 추가 실패 (HTTP ${response.status})${
          bodyText ? `: ${bodyText}` : ''
        }`
      )
    }

    // response.json()은 간헐적으로 파싱 오류를 내며(프록시/네트워크 이슈 등),
    // 이 경우 이벤트는 생성됐는데도 앱에서 "실패"로 오인할 수 있어 text 기반으로 처리합니다.
    const rawText = await safeReadResponseText(response)
    const data = safeJsonParse(rawText)
    let eventId = data?.id || null

    if (!eventId) {
      // 응답 파싱 실패/ID 누락 시, 방금 만든 이벤트를 캘린더에서 재조회하여 eventId를 확보합니다.
      const foundId = await findRecentlyCreatedEventId(accessToken, schedule, workplace)
      eventId = foundId || null
    }

    if (!eventId) {
      // 여기까지 오면 "생성은 되었을 수 있으나 ID를 확인하지 못한" 상태입니다.
      // 사용자에게는 확인을 유도하되, 에러 메시지는 구체화합니다.
      throw new Error(
        '캘린더 이벤트는 생성되었을 수 있으나, 이벤트 ID를 확인하지 못했습니다. ' +
          '구글 캘린더에서 등록 여부를 확인해주세요.'
      )
    }
    ...
    // 네트워크 오류(요청은 서버에 도달했을 수 있음) 등에서 "생성됨"인데 "실패"로 뜨는 케이스를 줄이기 위해
    // 한 번 더 캘린더 재조회로 확인을 시도합니다.
    const message = String(error?.message || '')
    const looksLikeNetworkError =
      error?.name === 'TypeError' ||
      message.includes('Failed to fetch') ||
      message.toLowerCase().includes('network')
    if (looksLikeNetworkError && accessToken && schedule && workplace) {
      try {
        const foundId = await findRecentlyCreatedEventId(accessToken, schedule, workplace)
        if (foundId) {
          return { success: true, eventId: foundId }
        }
      } catch (e) {
        // ignore verify failure, fall through to error
      }
    }
    ...
    ...(schedule?.id
      ? {
          extendedProperties: {
            private: {
              app: 'njob-manager',
              njobScheduleId: String(schedule.id),
              workplaceId: String(schedule.workplaceId || ''),
            },
          },
        }
      : {}),
...
async function findRecentlyCreatedEventId(accessToken, schedule, workplace) {
  const normalized = normalizeScheduleDateTimes(
    schedule?.date,
    schedule?.startTime,
    schedule?.endTime
  ...
  // 1) extendedProperties(private)로 우선 조회(가장 정확)
  const scheduleId = schedule?.id ? String(schedule.id) : ''
  if (scheduleId) {
    const byPrivate = await findEventIdByPrivateProperty(
      accessToken,
      scheduleId,
  ...
  // 2) 시간 범위 내에서 summary/start/end로 추정 매칭
  const expectedSummary = `${workplace?.name || ''} 근무`
  const expectedStart = `${normalized.startDate}T${normalized.startTime}`
  const expectedEnd = `${normalized.endDate}T${normalized.endTime}`

  // 생성 직후 일시적으로 목록 반영이 늦는 경우가 있어 짧게 재시도합니다.
  const retries = [0, 300, 700]
  for (const waitMs of retries) {
    if (waitMs) await delay(waitMs)
    const items = await listPrimaryEventsInRange(
      accessToken,
      addDays(normalized.startDate, -1),
    ...
    for (const ev of items) {
      ...
      if (!summary || !startDt || !endDt) continue
      if (summary !== expectedSummary) continue

      // 다양한 타임존 표현(Z, +09:00 등)을 고려해 "YYYY-MM-DDTHH:mm" 포함 여부로 비교
      if (!startDt.includes(expectedStart) || !endDt.includes(expectedEnd)) continue
...
async function listPrimaryEventsInRange(accessToken, startDate, endDate) {
  const timeMin = buildRfc3339Seoul(startDate, '00:00:00')
  const timeMax = buildRfc3339Seoul(endDate, '23:59:59')
  const url =
...
async function findEventIdByPrivateProperty(
  accessToken,
  scheduleId,
  startDate,
  endDate
) {
  const timeMin = buildRfc3339Seoul(addDays(startDate, -1), '00:00:00')
  const timeMax = buildRfc3339Seoul(addDays(endDate, 1), '23:59:59')
  const prop = `njobScheduleId=${scheduleId}`
  const url =
    `${CALENDAR_API_URL}/calendars/primary/events` +
    `?singleEvents=true&orderBy=startTime&maxResults=2500` +
    `&privateExtendedProperty=${encodeURIComponent(prop)}` +
    `&timeMin=${encodeURIComponent(timeMin)}` +
    `&timeMax=${encodeURIComponent(timeMax)}`
  ...
  const found = items.find((ev) => ev?.id)
